...
def compute_first_pass_fixation_count(
    ...
    """
    Number of first-pass fixations per word.
    Internal helper for SFD and sanity checks.
    """
    return (
        fix
        ...
        .group_by(["trial", "page", "word_idx"])
        .len()
        .rename({"len": "FPFC"})  # First-Pass Fixation Count
    )
...
def compute_first_fixation_duration(fix: pl.DataFrame) -> pl.DataFrame:
    return (
        ...
        .group_by(["trial", "page", "word_idx"])
        .first()
        .select(["trial", "page", "word_idx", "duration"])
        .rename({"duration": "FFD"})
    )
