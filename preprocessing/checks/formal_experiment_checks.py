...
def check_all_screens_logfile(
    ...
    for stimulus in stimuli:
        ...
        for question in stimulus.questions:
            if (
                f"{question.id}" not in stimulus_frame["page_number"].to_list()
...
def check_messages(
    ...
    for stim_id in stimuli_order:
        ...
        if not current_stimulus:
            raise ValueError(
                f"Stimulus with id {stim_id} not found in stimuli. "
        ...
        _extract_reading_time(
            current_stimulus.name,
            index_obligatory_break,
            last_msg_index,
            last_msg_timestamp,
            messages,
            index_next_stimulus,
        ...
        for page in current_stimulus.pages:
            for msg in msg_to_find:
                ...
                if current_pattern not in trial_messages_only:
                    ...
                    last_msg_index = (
                        trial_messages_only.index(current_pattern) + last_msg_index
                    )

        _check_rating_screens(trial_messages_only, report_file)

        _check_question_screens(
            current_stimulus, trial_messages_only, msg_to_find, pattern, report_file
        )

        _check_validation_screen(
            trial_messages_only, report_file, current_stimulus.name
        )
...
def _check_optional_screens(messages, messages_only, report_file):
    for optional_screen in OPTIONAL_SCREENS:
        indices = list(
            filter(
                lambda i: messages_only[i] == optional_screen, range(len(messages_only))
            )
        )
        if indices:
            _report_information(
            ...
            for index in indices:
                if (
                    optional_screen == "optional_break"
                    or optional_screen == "obligatory_break"
                ):
                    ...
                    while temp_index <= (index + 10):
                        ...
                        if temp_index < len(messages):
                            ...
                            if (
                                "optional_break_duration" in msg["message"]
                                or "obligatory_break_duration" in msg["message"]
                            ):
...
def _check_question_screens(
    current_stimulus, messages, msg_to_find, pattern, report_file
):
    for question in current_stimulus.questions:
        for msg in msg_to_find:
            if msg.startswith(
                "screen"
            ):  # get the cleaned question_id (removed zero at the start, if present)
