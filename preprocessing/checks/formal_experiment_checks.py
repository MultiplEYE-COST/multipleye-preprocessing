...
def check_all_screens_logfile(logfile: pl, stimuli: Stimulus | list[Stimulus], report_file: Path = None):
    ...
    for stimulus in stimuli:
        ...
        try:
            trial_id = logfile.filter((pl.col("stimulus_number") == str(stimulus.id))).item(0, "trial_number")  # get the trial number for the stimulus as ratingscreens don't have an entry in the stimulus_number column
        except:
            trial_id = logfile.filter((pl.col("stimulus_number") == str(float(stimulus.id)))).item(0, "trial_number")
        stimulus_frame = logfile.filter(
            (pl.col("trial_number") == trial_id)
        )
    ...
    for question in current_stimulus.questions:
        for msg in msg_to_find:
            ...
            if current_pattern not in messages:
                ...
                current_pattern = f"{msg}{pattern}_question_{float(stim_id)}{int(question_id)}"
                if current_pattern not in messages:
