...
def remap_wrong_pq_values(pq_data: dict) -> dict:
    ...
    possible_languages = [
        ...
        "use_language",
        "dominant_language",
        "additional_read_language_1",
        "additional_read_language_2",
        "additional_read_language_3",
        ...
        if f"{lang}_magazine_reading_time" in pq_data:
            for read in reading:
                ...
                if read == "academic_reading_time":
                    corrected_reading_times[key] = pq_data[
                        f"{lang}_magazine_reading_time"
                    ]
                    # print(f"Correcting {key} to {lang}_magazine_reading_time")
                elif read == "magazine_reading_time":
                    corrected_reading_times[key] = pq_data[
                        f"{lang}_newspaper_reading_time"
                    ]
