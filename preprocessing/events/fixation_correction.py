...
def _filter_gaze_events(events: pl.DataFrame, stimulus_name:str, page_name:str, event_type:str="fixation") -> pl.DataFrame:
...
def _filter_aois(stimulus, page_name) -> pl.DataFrame:
    ...
    return stimulus.text_stimulus.aois.filter(
        (pl.col("page") == f"{page_name}")
    )
...
def _get_lines_of_text_form_aois(aois: pl.DataFrame) -> list:
    ...
    if(len(heights) > 1):
        raise ValueError("Different heights for AOIs, cannot calculate line positions reliably")
    height = heights[0]
    return [line+height/2 for line in sorted(set(aois["top_left_y"]))]
    

def create_corrected_fixations_locations(events: pl.DataFrame, aois: pl.DataFrame, algorithm:str="chain"):
...
def _add_corrected_fixations_to_events(original_events: pl.DataFrame, corrected_fixations: list, algorithm: str, events_destination: pl.DataFrame) -> pl.DataFrame:
...
def add_corrected_fixations_for_page(stimulus, page, gaze_events: pl.DataFrame, algorithm:str="chain") -> pl.DataFrame:
    ...
    if events.select(pl.col("name").str.contains(f"fixation_corrected_{algorithm}").sum()).item()>0:
        raise ValueError(f"Fixation corrections with algorithm {algorithm} already exist for {stimulus_name}/{page_name} in the gaze events dataframe, cannot add corrected fixations again without removing the previous ones first. Remove them with the function remove_previous_fixation_corrections(gaze_events, algorithm)")

    aois = _filter_aois(stimulus, page_name)
    corrected_fixations = create_corrected_fixations_locations(events, aois, algorithm)
    gaze_events.frame = _add_corrected_fixations_to_events(events, corrected_fixations, algorithm, gaze_events.frame)
        

def add_corrected_fixations(sess, gaze_events: pl.DataFrame, algorithm:str="chain",verbose=0) -> pl.DataFrame:
    """Adds the corrected fixations for all stimuli and pages in the session to the gaze events dataframe.
    """

    if gaze_events.frame.select(pl.col("name").str.contains(f"fixation_corrected_{algorithm}").sum()).item()>0:
        raise ValueError(f"Fixation corrections with algorithm {algorithm} already exist in the gaze events dataframe, cannot add corrected fixations again without removing the previous ones first. Remove them with the function remove_previous_fixation_corrections(gaze_events, algorithm)")

    for stimulus in sess.stimuli:
        if verbose==1: print(stimulus.name)
        for page in stimulus.pages:
            if verbose==1:print(page.number,end=",")
            add_corrected_fixations_for_page(stimulus, page, gaze_events, algorithm)
        if verbose==1:print()

def remove_previous_fixation_corrections(gaze_events, algorithm:str="chain") -> pl.DataFrame:
    gaze_events.frame = gaze_events.frame.filter(~pl.col("name").str.contains(f"fixation_corrected_{algorithm}"))
    return gaze_events
