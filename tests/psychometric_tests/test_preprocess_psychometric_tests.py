...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([float("nan"), 100, 200, 300], [float("nan"), 1, 0, 1], False, 200.0, 2 / 3,
         3),
        ([float64("nan"), 100, 200, 300], [float64("nan"), 1, 0, 1], False, 200.0,
         2 / 3, 3),
        ([100, 200, 300, float64("nan")], [1, 0, 1, float64("nan")], False, 200.0,
         2 / 3, 3),
...
def test__reaction_time_accuracy_grouped(df_builder, correct_only, expectations):
    ...
    res = _reaction_time_accuracy(
        df, reaction_time_col="rt", correctness_col="correctness", group_by_col="cond",
...
def test__reaction_time_accuracy_grouped_missing_column(group_by_col, expect_error):
    ...
    if expect_error:
        ...
        out = _reaction_time_accuracy(df, "rt", "correctness",
...
def test__find_one_filetype_with_columns(
        tmp_path: Path, make_text_file, files, required_cols, allow_nan,
        expect_error_msg, check
):
...
def test_preprocess_stroop_errors(tmp_path: Path, make_text_file, header, body,
                                  error_msg):
...
def test_preprocess_flanker_errors(tmp_path: Path, make_text_file, header, body,
                                   error_msg):
...
def test_preprocess_plab_errors(tmp_path: Path, make_text_file, header, body,
                                error_msg):
...
def test_preprocess_wikivocab_errors(tmp_path: Path, make_text_file, header, body,
                                     error_msg):
