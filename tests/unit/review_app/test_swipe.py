...
def test_remove_plot_judgment(tmp_path, monkeypatch):
    ...
    from review_app.services.swipe import save_plot_judgment, remove_plot_judgment, load_judgments
...
def _make_swipe_stats(sessions, plot_map, judgments_dict, monkeypatch):
    ...
    def _fake_list_plot_data(_dcn, sid):
        return [{"name": p, "url": f"/files/{p}.png", "stimulus": p, "page": "", "activity": ""}
...
@pytest.mark.parametrize("sessions,plot_map,judgments,expected", [
    pytest.param(
        [], {}, {},
        {"plots_judged": 0, "total_plots": 0, "keep": 0, "flag": 0, "skip": 0,
         "sessions_complete": 0, "sessions_skipped": 0, "total_sessions": 0, "progress_pct": 0},
        id="empty",
    ),
    pytest.param(
        ...
        {"plots_judged": 4, "total_plots": 4, "keep": 2, "flag": 1, "skip": 1,
         "sessions_complete": 2, "sessions_skipped": 0, "total_sessions": 2, "progress_pct": 100.0},
    ...
    pytest.param(
        ...
        {"plots_judged": 1, "total_plots": 4, "keep": 1, "flag": 0, "skip": 0,
         "sessions_complete": 0, "sessions_skipped": 1, "total_sessions": 2, "progress_pct": 25.0},
    ...
    pytest.param(
        ...
        {"plots_judged": 3, "total_plots": 4, "keep": 1, "flag": 1, "skip": 1,
         "sessions_complete": 1, "sessions_skipped": 1, "total_sessions": 2, "progress_pct": 75.0},
    ...
    pytest.param(
        ...
        {"s1": ["p1", "p2"], "s2": ["p3", "p4"]},
        {},
        {"plots_judged": 0, "total_plots": 4, "keep": 0, "flag": 0, "skip": 0,
         "sessions_complete": 0, "sessions_skipped": 0, "total_sessions": 2, "progress_pct": 0.0},
        id="none_judged",
    ),
    pytest.param(
        [FakeSession("s1"), FakeSession("s2")],
        ...
        {"plots_judged": 2, "total_plots": 2, "keep": 1, "flag": 1, "skip": 0,
         "sessions_complete": 2, "sessions_skipped": 0, "total_sessions": 2, "progress_pct": 100.0},
    ...
    pytest.param(
        ...
        {"plots_judged": 1, "total_plots": 1, "keep": 1, "flag": 0, "skip": 0,
         "sessions_complete": 1, "sessions_skipped": 0, "total_sessions": 2, "progress_pct": 100.0},
        id="session_with_no_plots_excluded",
    ),
])
def test_swipe_stats_edge_cases(sessions, plot_map, judgments, expected, tmp_path, monkeypatch):
    monkeypatch.setattr("review_app.config.REVIEW_DATA_DIR", tmp_path)
    stats = _make_swipe_stats(sessions, plot_map, judgments, monkeypatch)
    for key, val in expected.items():
        assert stats[key] == val, f"Mismatch for {key}: expected {val}, got {stats[key]}"
...
def test_parse_plot_name():
    ...
    assert _parse_plot_name("main_sequence") == {
        "stimulus": "Main Sequence", "page": "", "activity": ""
    }
    assert _parse_plot_name("stim1_2") == {
        "stimulus": "stim1", "page": "page 2", "activity": ""
    }
    assert _parse_plot_name("stim1_q3") == {
        "stimulus": "stim1", "page": "question 3", "activity": ""
    }
